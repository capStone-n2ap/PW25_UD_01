...
def create_model(num_labels=14, num_frames=8, dropout_rate=0.1):
...
def objective(trial):
    ...
    learning_rate = trial.suggest_float('learning_rate', 2.65e-5, 8.24e-5, log=True)
    batch_size = trial.suggest_categorical('batch_size', [8, 16])
    weight_decay = trial.suggest_float('weight_decay', 5.009e-5, 1.05e-3, log=True)
    dropout_rate = trial.suggest_float('dropout_rate', 1.16e-1, 2.72e-1)
    num_epochs = trial.suggest_int('num_epochs', 10, 15)
    warmup_ratio = trial.suggest_float('warmup_ratio', 9.76e-02, 1.40e-1)
    ...
    # Data paths 
    train_path = r"C:\Users\Student\Desktop\CML_HPO\dataset\IPN\train"
    val_path = r"C:\Users\Student\Desktop\CML_HPO\dataset\IPN\val"
    ...
    model = create_model(num_labels=14, num_frames=num_frames, dropout_rate=dropout_rate)
...
def main():
    ...
    storage_name = "sqlite:///testingBayesianMAML3_clg_syst_optuna_IPN_finetuned_percentile_pruning.db"    
    sampler_file = "testingBayesianMAML3_clg_syst_optuna_IPN_finetuned_percentile.pkl"  # File to save sampler state
    ...
    study = optuna.create_study(
        study_name="testingBayesianMAML3_timeSformer_gestureClassification_clg_syst_IPN_finetuned_percentile_pruning",
        direction="maximize",
        sampler=sampler,
        pruner=pruner,
        storage=storage_name,
